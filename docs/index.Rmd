...
Above we have loaded the empirical data, which is under NDA. However, one can use synthetic data to evaluate our approach,

```{r, eval=FALSE}
d <- readRDS("data/data.rds")
```

The above line is currently not executed, since we run this analysis with the empirical data. However, if one wants to evaluate our approach, then using synthetic data is the only way, and simply executing the `setup` chunk in the source code, and then loading the synthetic data, above, will allow you to then continue executing the below analysis as-is.
