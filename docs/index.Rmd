...
To conclude what we've noticed so far: `Priority`, `Critical feature`, `Stakeholders`, and `Dependencies` are clearly significant on $\text{CI}_{95\%}$, while for `Business value` and `Customer value` some categories are significant. `Architecture involvement` and `Key customers` are not significant.
