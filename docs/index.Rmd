...
subtitle: "Reproducibility package"
...
The data we load above is the empirical data, which is under NDA and unfortunately not generally available. In order to load the synthetic data we instead load it directly into memory, i.e., `d <- readRDS("data/data.rds")`, but we do it later down below, so please stay with us for the time being.
...
Each row (requirement) has a unique `ID`. The `State`, which is our outcome (dependent variable), shows how far the feature got in the process. It is of an ordered categorical type, which should be modeled with some type of cumulative likelihood. There are seven categories:
...
Finally, we set correct data types on all predictors.
...
Since our outcome is, in all practical sense, a Likert scale variable using an adjacent category (acat) model is really not an option.^[The acat model is a common ordinal model in item-response theory] That leaves us with two other types of models: cumulative and sequential. We'll make sure to try both and also model, when appropriate, predictors as monotonic (cumulative) or category-specific (sequential) effects [@burkner20monotonic].

First we sample a model that only has a population-level intercept (our null model, $\mathcal{M}_0$). Then we sample two models using a cumulative likelihood (without and with monotonic predictors, i.e., $\mathcal{M}_1$ and $\mathcal{M}_2$, respectively) [@burkner19ordinal]. Finally, we sample two models using the  Once we've sampled our models we then use LOO to compare each model's relative out of sample prediction capabilities [@vehtari17loo].
...
# cumulative model with our covariates
M1 <- brm(State ~ 1 + prio_s + crit + b_val + c_val + sh_s + kc_s + dep + arch,
          family = cumulative, data = d, cores = 4, chains = 4, 
          threads = threading(4))

# since it's cumulative likelihood we could model ordered categorical predictors
# as monotonic
...
# seq. model w/ category-specific effects
...
M5 <- brm(State ~ 1 + prio_s + crit + b_val + c_val + 
                 sh_s + kc_s + dep + arch,
          family = acat(), data = d, cores = 4, chains = 4, 
          threads = threading(4))

# w/ category-specific effects
M6 <- brm(State ~ 1 + prio_s + crit + cs(b_val) + cs(c_val) + 
                 sh_s + kc_s + dep + cs(arch),
          family = acat(), data = d, cores = 4, chains = 4, 
          threads = threading(4))
```
...
```{r loo_comp, cache=TRUE}
# run it with reloo=TRUE, but the results are still ok
(l <- loo_compare(loo(M0), loo(M1), loo(M2), loo(M3), loo(M4), loo(M5), loo(M6)))
```

`LOO` puts $\mathcal{M}_4$ as no. 1. If we assume a $z_{\mathrm{99\%}}$-score of $2.58$ it's clear that zero is not in the interval and that $\mathcal{M}_4$ has an advantage, i.e., $\mathrm{CI}_{z_{99\%}}$ [`r round(l[2,1] + c(-1,1) * l[2,2] * 2.58, 2)`]. We can also conclude this matter by saying that adding predictors to $\mathcal{M}_0$ *clearly* has a significant effect, but adding monotonic effects to our model does not (but adding category specific effects definitely does).
...
Above we have used the empirical data, which is under NDA. However, one can use synthetic data to evaluate our approach,

```{r, eval=FALSE}
d <- readRDS("data/data.rds")
```

The above line is currently not executed, since we will continue to run this analysis with the empirical data. However, if one wants to evaluate our approach, then using synthetic data is the only way. Simply executing the `setup` chunk (first chunk) in the source code, and then loading the synthetic data, above, will allow you to then continue executing the below analysis as-is.

In the remainder of this analysis we show only the results from using the empirical data.
...
We have $15$ $\beta$ and $5$ $\alpha$ parameters that we need priors. Since we use a $\logit$ link for our model (to transform a linear model into a probability) it is hard to conceptually grasp the effect of our priors, i.e., we need to visualize them.
...
# N(0,0.5) might seem very strict, but (15*0.5)^2 = 56.25
p$prior[1] <- "normal(0, 0.5)"

# Our priors on alpha (five of them; one for each crosspoint (border) between 
# Likert scale 1,...,6)
p$prior[17] <- "normal(0, 5)"
```
...
pp_check(M_prior, type = "bars", nsamples = 250)
```

If we sample from our prior predictive distribution, $y_{\mathrm{rep}}$, and plot it together with our empirical data, $y$, we'll see how the priors cover our data. As is evident (see right), by looking at the intervals, our priors seems to be relaxed and should not influence the outcome, and in our case we have $11100$ rows of data so priors should affect our outcome very little. The priors are also set up so more probability mass is set to the earlier stages since it's basically a survival model, i.e., a feature needs to survive in order to reach the next stage.
...
Diagnostics such as divergences, tree depth and enery looks good. Additionally, our $\widehat{R}$ and effective sample size (ESS) look good.
...
# Check rhat and ESS
if(max(rhat(eval(M)), na.rm=T) >= 1.01) 
  print("Warning: Rhat >= 1.01")
if(min(neff_ratio(eval(M)), na.rm=T) <= 0.2) 
...
Examining the above plot, from bottom to top, we can say that on the $95$%-level, the first three parameters are clearly positive or negative and do not cross $0$. The fourth parameter, `Key customers`, is not significant, with the following $95$% credible intervals [`r round(fixef(M)[9,3], 2)`, `r round(fixef(M)[9,4], 2)`]. The fifth parameter, `Dependencies`, is significant (positive) [`r round(fixef(M)[10,3], 2)`, `r round(fixef(M)[10,4], 2)`]. For our three category specific parameters `Business value`, `Customer value`, and `Architects' involvement` we see that some parameters are significant, i.e., for `Business value` the parameters $[1]$ and $[4]$ and for `Customer value` parameter $[3]$. There are no significant parameters in `Architects' involvement`.
