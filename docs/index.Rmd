...
options(brms.backend="cmdstanr") # run with cmdstan(r)
```
...
d$Business.value <- as.integer(d$Business.value)
d$Customer.value <- as.integer(d$Customer.value)
d$Architects.involvement <- as.integer(d$Architects.involvement)
...
* <font style="font-family: serif">Cumulative</font> (single underlying continuous variable) [@samejima97].
* <font style="font-family: serif">Adjacent-category</font> (mathematically convenient) [@agresti10].
* <font style="font-family: serif">Sequential</font> model (higher response category is possible only after all lower categories are achieved) [@tutz90].

We'll make sure to compare all and also model, when appropriate, predictors as monotonic effects (for the <font style="font-family: serif">Cumulative</font> likelihood) or category-specific effects (for the <font style="font-family: serif">Sequential</font> likelihood). The reason we want to use monotonic or category-specific modeling of predictors is that predictor categories will not be assumed to be equidistant with respect to their effect on the outcome [@burkner20monotonic].
...
# Check divergences, tree depth, energy
rstan::check_hmc_diagnostics(eval(M3)$fit)
