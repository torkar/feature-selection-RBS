...
When using `Stan` we have a plethora of diagnostics, which we should utilize to ensure validity and efficiency in our sampling. Below we summarize the ones we use.^[https://mc-stan.org/misc/warnings.html]
...
Tree depth warnings are not a validity concern but rather an efficiency concern. Reaching the maximum tree depth indicates that the sampler is terminating prematurely to avoid long execution time. 
...
First, let's compare the model where we did *not* use category-specific effects, with the model which did use them. On the left side we see $\mathcal{M}_4$, while on the right side we see the 'final' model we've settled on, designated $\mathcal{M}$. 
