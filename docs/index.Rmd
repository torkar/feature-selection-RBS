...
The data we load above are the empirical data which is under NDA and unfortunately not generally available. In order to load the synthetic data we instead load it directly into memory, i.e., `d <- readRDS("../data/data.rds")`, and then you can continue run the below commands as-is (with slightly different results obviously).
