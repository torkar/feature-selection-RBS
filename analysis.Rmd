...
If we would be interested in refining our model for out of sample prediction purposes we should conduct variable selection. However, in this particulare case we are interested in each predictor's effect, so we'll keep them all.
...
First, we set weakly regularizing priors on $\beta$, while we set the same on our monotonic predictors. Then, after looking at different levels of priors, we can conclude that the following priors are suitable for this model.
